...
- **Why.** Doctor shipped the "is this install broken?" diagnostic but there was no paved path *to* a working install — the old README was a five-line stub that implied `bun install && bun run dev` would just work. The punch list called out docs as the third prong of M12 polish. Also: the Google Cloud Console steps are the one part of grace someone can't reverse-engineer from the codebase, so they need to be written down.
...
- **env bypass.** Reads `process.env` directly rather than routing through `@grace/env/server` — zod's `.parse()` throws on missing `GOOGLE_OAUTH_CLIENT_ID`, and the whole point of doctor is to *report* that absence gracefully. Also cheaper: `.env` resolution already ran via Bun's auto-load before the script starts.
