...
### 2026-04-19 (latest) — M12 network online/offline detection

- **Why.** The IDLE supervisor already reconnects on socket close, but it reacts at the _outcome_ level — the socket has to fail and the backoff has to elapse before we try again. When the machine wakes from sleep or WiFi comes back after a long drop, that's up to 60s of staring at stale mail before the next attempt. The punch list called this out as the last behavioral M12 item; docs, doctor, and oauth-logout are done, so this closes the loop on the "resilience" half of the milestone.
- **`packages/mail/src/network-monitor.ts`.** Periodic HEAD/GET against `https://www.gstatic.com/generate_204` — gstatic is the canonical connectivity-check endpoint (tiny 204 response, designed for exactly this), so we're not abusing a random site. Two intervals: 30s while online (the supervisor is already reactive, this is really just "did we wake up?"), 5s while offline (want fast recovery). 5s timeout per probe. Transitions fire `onStateChange(status, prev)` only on actual flips, not on every successful probe — callers don't need to dedupe.
- **`supervisor.kick(reason?)`.** New method on `IdleSupervisor`. If a reconnectTimer is pending (supervisor is in backoff), clears it and calls `connectLoop()` immediately; returns `true`. No-ops (returns `false`) when there's no timer — the supervisor is either currently connecting, watching, or stopped, and none of those want interrupting. Status isn't set directly from `kick` because `connectLoop()` sets `connecting` itself as its first act, so a double-emit would be redundant.
- **`folderManager.kickAll(reason?)`.** Fan-out helper: iterates supervisors, returns the count that were actually kicked. The return value is logged so tail-watchers can see "kicked 2 idle supervisor(s) out of backoff" vs a silent no-op when nothing was waiting.
- **API singleton at `packages/api/src/network-monitor.ts`.** Mirrors the pattern of `folder-manager.ts` and `imap-action.ts` — one `startNetworkMonitorSingleton()` entry point, idempotent, returns the monitor. Wires `onStateChange` to publish `network.status` bus events and to kick the folder-manager out of backoff on `offline → online` transitions specifically. `unknown → online` (fresh start) doesn't kick because there's nothing to kick yet; `* → offline` doesn't kick because there's nothing to connect to. The log line differentiates `confirmed` (first probe succeeded) from `restored` (we saw a real outage end).
- **Bus event.** `network.status { state: "online" | "offline", reason? }`. TUI doesn't render it yet — that's queued as a deferred banner on `idle.status` + `network.status` together — but consumers can subscribe today.
- **Server wire.** `apps/server/src/index.ts` calls `startNetworkMonitorSingleton()` right after `ensureInboxIdle()` on boot, and `stopNetworkMonitorSingleton()` on SIGTERM before stopping the folder manager. Probe fires on start so the initial state is known within ~5s instead of waiting the full 30s interval.
- **Why gstatic, not a more direct Gmail reachability probe.** We could HEAD `https://gmail.com` or TCP-connect `imap.gmail.com:993`, but (1) gstatic is specifically designed for this and returns a 204 with no body so it's cheaper, (2) a Gmail-specific outage is extraordinarily rare and the IMAP supervisor will detect it within its own reconnect cycle anyway, (3) using a Google endpoint isn't meaningfully different from already depending on Google infra for the whole app. If Anthropic or similar ever becomes a dep, adding a second probe target is trivial.
- **Deferred.** TUI banner showing offline / reconnecting-in-Xs composite state. The wire now carries both signals; rendering them is a few lines in the header component but out of scope for this iteration.

### 2026-04-19 — M12 README + SETUP.md

- **Why.** Doctor shipped the "is this install broken?" diagnostic but there was no paved path _to_ a working install — the old README was a five-line stub that implied `bun install && bun run dev` would just work. The punch list called out docs as the third prong of M12 polish. Also: the Google Cloud Console steps are the one part of grace someone can't reverse-engineer from the codebase, so they need to be written down.
- **README.md rewrite.** Replaced the stub with: one-paragraph pitch, quick-start for the already-set-up case with a prominent pointer to SETUP.md for the fresh case, "What works" bullet list that mirrors the shipped feature set (real-time push, hybrid bodies, two-phase search, threaded reply, triage, per-folder IDLE, reconnect supervisor), condensed keybind cheat-sheet grouped by mode, architecture ASCII, scripts table, repo layout, and data-locations section (Keychain service `grace`, `~/.grace/grace.db`, `~/.grace/bodies/`, `~/.grace/drafts/`). Keybind strings pulled from the live registry in `apps/tui/src/keybind/index.tsx` + the help-dialog labels so they don't drift.
- **SETUP.md (new).** Numbered sections: prerequisites (macOS + Bun 1.3+ + meta-key-forwarding terminal + optional w3m), Google Cloud Console setup (enable Gmail API → OAuth consent screen → Desktop-app client), clone + install, `.env` config, `oauth:login` flow semantics (PKCE + loopback + Keychain + `mail.google.com/` scope rationale), `doctor` section-by-section, everyday scripts, and a troubleshooting playbook. Testing-mode 7-day refresh-token expiry called out in two places (near the GCP step + near the `oauth:login` step) because it's the #1 thing that will confuse a fresh user a week later.
- **Why scope is `https://mail.google.com/` not `gmail.modify`.** Documented in SETUP.md because it's a common GCP-console footgun — the consent screen lets you add the narrower scopes, but IMAP/SMTP over XOAUTH2 specifically requires the full-access scope. Someone who picks `gmail.modify` from the GCP scopes picker will get a confusing "auth failed" at sign-in time; the prose now warns them off.
- **Troubleshooting entries cover real incidents.** "Too many simultaneous connections" (the ps-grep to find straggler daemons came straight from what we actually run), IDLE-never-reconnects (points at the supervisor module so an issue-filer knows where to look), "no plain-text part" (browser-eject fallback), refresh-token expiry, and a clean-wipe one-liner (`oauth:logout && rm -rf ~/.grace`).
- **Not touched.** `prd.md` and `refresh.md` — per the ralph hard rule, docs go into README + SETUP. Kept the prose terse and technical; no "why grace exists" philosophizing (`prd.md` already does that).

### 2026-04-19 — M12 `grace oauth logout` CLI
...
- **env bypass.** Reads `process.env` directly rather than routing through `@grace/env/server` — zod's `.parse()` throws on missing `GOOGLE_OAUTH_CLIENT_ID`, and the whole point of doctor is to _report_ that absence gracefully. Also cheaper: `.env` resolution already ran via Bun's auto-load before the script starts.
- **keychain check.** `loadActiveAccount()` → `loadTokens(email)` → verifies `refreshToken` present, access-token `expiresAt` vs now (3 bands: >1m OK, <1m or expired → warn that auto-refresh will fire on next use), scope contains `https://mail.google.com/`. The scope check is the one that surfaces a surprisingly common fresh-install failure — old tokens from before we switched from `gmail.modify` to full IMAP scope.
- **imap is the expensive check.** Runs only if active account + OAuth env are all present. Pipeline: `getFreshAccessToken` → `createImapClient` → `connect` → `list` → open INBOX → report `exists`/`uidNext` → `logout`. The 15-conn Gmail cap hint fires as a detail line when the error message matches, so `doctor` immediately points at "kill stray dev daemons" instead of making the user google the error.
- **daemon check is a warn, not fail.** Many people will run `doctor` _before_ starting the daemon for the first time. A hard-fail there would be unhelpful. Instead it reports "not reachable — start with `bun run dev:server`" and moves on. A bonus check confirms the `name` field is `grace` so a port-collision with something else doesn't read as a happy health-check.
- **Verified end-to-end locally.** Output on this machine correctly identifies: `.env` present, both OAuth vars set, `ANTHROPIC_API_KEY` absent (warn → M11 disabled), active account, refresh token present, access token expired (warn, auto-refresh fires), scope correct, db present with 2 folders / 1004 messages / 94 bodies, w3m missing (warn), daemon not running (warn), live IMAP handshake + 24 mailboxes visible + INBOX stats + clean logout. Runtime ~2s including the IMAP round-trip.

### 2026-04-19 — M7 per-folder IDLE

- **Why.** INBOX was the only folder with live push. Switching to `Work` or `Important` required pressing `r` to see new mail — fine when the sidebar is a demo, annoying when it's your inbox. The M12 supervisor made the fix mechanical: multiply it.
- **`packages/mail/folder-manager.ts`.** `startFolderManager({email, creds, db, maxConcurrent=4, onNewMessage, onStatus, onError, debug})` returns `{ ensure, release, list, has, stopAll }`. `ensure(folderName)` no-ops if already watching and returns `{ started: false, reason }` if the cap is reached — it doesn't throw, because a hit cap is a degraded-but-functional state (backfill + manual `r` still work). Supervisor callbacks are wrapped to inject the folder name into `onNewMessage` / `onError`; `onStatus` already carries `folder` from `IdleSupervisorStatus`.
- **Why cap at 4.** Gmail allows 15 concurrent IMAP connections per account. Subtract 1 action client (permanent), +1-2 for transient bootstrap/backfill clients when activating a fresh folder, +1 headroom for SMTP. 4 IDLE supervisors plus peaks fits comfortably inside the envelope; 5+ starts risking the "too many simultaneous connections" error under dev-reload churn. Bump via `maxConcurrent` if real usage proves conservative.
- **Singleton in `packages/api/src/folder-manager.ts`.** Same pattern as `imap-action.ts`: lazy-init on first `getFolderManager()`, reads `loadActiveAccount()` + `requireGoogleOAuth()` once. Returns `null` when no account is signed in; `ensureFolderIdle(folderName)` surfaces that to callers by returning `null` rather than throwing. Bus callbacks live inside the singleton — publishers of `mail.received` / `idle.status` no longer need to duplicate the `state === "idle" ? "connecting"` mapping or the `delayMs/reason` spread logic.
- **Server boot simplified further.** `apps/server/src/index.ts` no longer holds its own supervisor variable; it calls `ensureFolderIdle("INBOX")` and `stopFolderManager()` at shutdown. `kickBackfill(folderName)` extracted so per-folder activations can reuse it (backfill is per-folder by folder-name set; unchanged behavior for INBOX). `formatImapError` moved into the singleton since that's where the `idle:` logs now live.
- **Activate route wires `ensureFolderIdle` in.** After `bootstrapFolder` completes for a user-switched sidebar folder, the route calls `ensureFolderIdle(folderName)` and logs the result (`started IDLE for X` / `already watching X` / `idle supervisor cap (4) reached; skipping X`). If the cap bites, the folder still backfills normally — it just won't get live push until the user releases a slot (or we add LRU eviction).
- **Regression surface.** INBOX startup path is identical (same supervisor primitive, same `mail.received` / `idle.status` bus events, same backfill). The deferred TUI status banner was already not shipped, so there's nothing to refactor for multi-folder status.
- **Deferred.** LRU eviction when the cap is hit (today: first-come-first-served). Explicit per-folder release when user switches away — would let active supervisors track user intent, but adds churn if they flip between folders quickly. Wait for real friction before implementing.

### 2026-04-19 — M12 idle reconnect with backoff
...
- **Row stays height=1.** Pills are `flexShrink=0` `<text>` fragments sharing the subject's flex budget — subject truncates first when labels are present, which is the right trade-off (labels _are_ meta about the subject).
...
- **Per-folder IDLE capped at 4 concurrent supervisors.** INBOX is always on; other folders get live push when you first activate them from the sidebar (up to 3 more simultaneously). Beyond that the fifth folder still bootstraps + backfills but won't push new mail until a slot opens — no LRU eviction yet. See plan.md M7 "Deferred".
...
M10 — command palette is already effectively shipped as part of the refresh primitives (`:` opens a fuzzy `DialogSelect` over registered commands). M12 is behaviorally complete (supervisor reconnect + network monitor + doctor + oauth-logout + docs); only a TUI banner for `idle.status` / `network.status` remains as polish. M11 — Claude features (summarize, draft, NL-select) is next, blocked on `ANTHROPIC_API_KEY` in `.env`. M8 and M9 are feature-complete.
