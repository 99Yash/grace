...
- **M7-05 ✅** Per-folder IDLE via `packages/mail/folder-manager.ts` — `startFolderManager` multiplies `startIdleSupervisor` across folders with a concurrency cap (`DEFAULT_MAX_CONCURRENT_IDLE = 4`) to leave Gmail conn budget for the action client + transient bootstrap/backfill. Singleton lives in `packages/api/src/folder-manager.ts`; lazy-initialized when an account exists; emits per-folder `mail.received` / `idle.status` events. Server boot ensures INBOX; `POST /api/folders/:name/activate` calls `ensureFolderIdle(folderName)` after bootstrap so activating a sidebar folder also pushes its new mail. Over-cap requests no-op with a `reason` string logged — they still get bootstrap + backfill, just not live push.
- **Deferred** LRU eviction when the cap is hit (today it's first-come-first-served — most sessions hit 1-3 folders so the cap bites rarely). TUI surfacing of idle.status per folder.
...
- **M12-06 ✅** `README.md` rewritten + new `SETUP.md`. README now covers what ships (real-time push, hybrid bodies, two-phase search, threaded reply, triage, per-folder IDLE, reconnect supervisor), a condensed keybind cheat-sheet, the scripts table, repo layout, and data locations (Keychain service, `~/.grace/` tree). SETUP.md is the fresh-install walkthrough — Google Cloud Console steps (OAuth consent + Desktop-app client creation), `.env` layout, `bun run oauth:login` semantics (PKCE loopback, `mail.google.com/` scope, Testing-mode 7-day expiry caveat), `bun run doctor` section-by-section explainer, everyday scripts, and a troubleshooting playbook (Gmail conn-cap kills, HTML-only mail fallbacks, token refresh behavior, clean-wipe recipe).
- **M12-07 ✅** Network online/offline detection — `packages/mail/src/network-monitor.ts` runs a periodic probe against `https://www.gstatic.com/generate_204` (30s interval online, 5s offline, 5s per-probe timeout). State transitions fan out as `network.status` bus events; on offline→online the API singleton calls `folder-manager.kickAll()` which invokes a new `supervisor.kick(reason)` on each IDLE supervisor, cancelling its pending backoff timer so a laptop waking from sleep reconnects immediately rather than waiting up to 60s. Singleton lives at `packages/api/src/network-monitor.ts`; started on server boot alongside `ensureInboxIdle()`, stopped on SIGTERM.
- **Deferred** TUI banner reflecting `idle.status` / `network.status` (reconnecting + countdown, offline indicator).
